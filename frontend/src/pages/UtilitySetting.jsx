...
const ConstantPanel = ({ constants, onClose, onSaved }) => {
  ...
  const handleEdit = (id) =>
    setItems((p) => p.map((c) => (c.id === id ? { ...c, _editing: true } : c)));
  const handleChange = (id, v) =>
    setItems((p) => p.map((c) => (c.id === id ? { ...c, _val: v } : c)));

  const handleSave = async (item) => {
    if (!item._val || Number(item._val) <= 0) return;
    setSaving(item.id);
    try {
      await constantService.updateConstant(item.id, {
        ...item,
        cost: Number(item._val),
      });
      setItems((p) =>
        p.map((c) =>
          c.id === item.id
            ? { ...c, cost: Number(item._val), _editing: false }
            : c,
        ),
      );
      onSaved?.();
  ...
  const isElecItem = (item) =>
    item.subject?.toLowerCase().includes("electricity") ||
    item.subject?.includes("ไฟ");
    ...
    >
      ...
      >
        ...
        <div className="px-7 py-5 space-y-3 max-h-[55vh] overflow-y-auto">
          ...
          {items
            .filter((item) => item.subject !== "priorityMeter")
            .map((item) => {
...
const ConfirmModal = ({
  ...
  const elecConst = utilityConsts.find(
    (c) =>
      c.subject?.toLowerCase().includes("electricity") ||
      c.subject?.includes("ไฟ"),
  );
  const waterConst = utilityConsts.find(
    (c) =>
      c.subject?.toLowerCase().includes("water") || c.subject?.includes("น้ำ"),
  );
  ...
  const RateInput = ({
    ...
    <div>
      <div className="flex items-center gap-2 mb-2">
        <Icon
          size={13}
          className={color === "orange" ? "text-orange-500" : "text-blue-500"}
          fill="currentColor"
        />
  ...
  const ConstToggle = ({
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <p
          className={`font-black text-sm ${linked ? (color === "orange" ? "text-orange-800" : "text-blue-800") : "text-gray-600"}`}
        >
          {label}
        </p>
        {constItem && (
  ...
  return (
    ...
    >
      <div
        className="bg-white rounded-[40px] w-full max-w-md shadow-2xl overflow-hidden flex flex-col max-h-[92vh]"
        onClick={(e) => e.stopPropagation()}
      >
        {/* header */}
        <div
          className={`px-7 py-5 ${applyMode === "contract" ? "bg-orange-50" : "bg-purple-50"}`}
        >
          ...
          <p className="text-xs font-bold text-gray-500 mt-1">
            ...
            {applyMode === "contract" && noContract.length > 0 && (
              <span className="text-orange-500 ml-1">
                · {noContract.length} ห้องไม่มีสัญญา → บันทึก Note ห้องแทน
              </span>
            )}
          </p>
          {/* preview rooms */}
          ...
          {applyMode === "constant" && (
            <>
              ...
              <ConstToggle
                ...
                color="blue"
                linked={linkWater}
                onToggle={() => setLinkWater((p) => !p)}
                constItem={waterConst}
              />
    ...
    >
      <div className="flex items-center justify-between mb-1.5">
        <div className="flex items-center gap-2">
          ...
          />
          <span
            className={`text-xs font-black ${val !== null ? (color === "orange" ? "text-orange-600" : "text-blue-600") : "text-gray-400"}`}
          >
            {label}
          </span>
        </div>
      ...
      {val === null ? (
        ...
        <p
          className={`text-sm font-bold ${color === "orange" ? "text-orange-700" : "text-blue-700"}`}
        >
          {val}
        </p>
      )}
...
const LazyFloor = React.memo(
  ({
    ...
    return (
      <div
        ref={ref}
        className="bg-gray-50 p-4 sm:p-6 rounded-3xl border border-gray-100"
      >
            ...
            {rooms.map((room) => {
              ...
              return (
                <div
                  ...
                  title={
                    disabled
                      ? "ห้องนี้ไม่มีสัญญา Active/Reserved/Expired"
                      : undefined
                  }
...
const UtilityRateSetting = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      setUtilityConsts(
        allConst.filter((c) => c.category?.toLowerCase() === "utility"),
      );
  ...
  /* selection */
  const isRoomSelectable = useCallback(
    (room) => applyMode === "constant" || room._hasActiveContract,
    [applyMode],
  );
    ...
    (floor) => {
      setSelectedIds((prev) => {
        ...
        (roomsByFloor[floor] || []).forEach((r) => {
          if (isRoomSelectable(r)) next.add(r.roomId);
        });
        return next;
      });
    },
  ...
  const selectAllVisible = useCallback(
    () =>
      setSelectedIds(
        new Set(filteredRooms.filter(isRoomSelectable).map((r) => r.roomId)),
      ),
  ...
  const handleTogglePriority = async () => {
    ...
    try {
      const newNote =
        priorityMeter.note === "constant" ? "contract" : "constant";
      ...
      showToast(
        `เปลี่ยนลำดับการคำนวณเป็นแบบ ${newNote === "constant" ? "Constant" : "สัญญา"} แล้ว`,
        "success",
      );
    } catch (e) {
      console.error(e);
  ...
  const handleConfirm = async ({
    ...
    try {
        ...
        try {
          if (applyMode === "contract") {
            if (room._contract) {
              ...
              let cNote =
                c.note || c.Note || c.attachedFile || c.AttachedFile || "";
              if (elecRate)
                cNote = replaceTag(cNote, "ค่าไฟ", `${elecRate}฿/หน่วย`);
              if (waterRate)
                cNote = replaceTag(cNote, "ค่าน้ำ", `${waterRate}฿/หน่วย`);
              await contractService.putContract(room._contractId, {
                ...
                InitialElectricUnit:
                  c.initialElectricUnit || c.InitialElectricUnit || 0,
              ...
              if (elecRate)
                rNote = replaceTag(rNote, "ค่าไฟ", `${elecRate}฿/หน่วย`);
              if (waterRate)
                rNote = replaceTag(rNote, "ค่าน้ำ", `${waterRate}฿/หน่วย`);
              await roomService.updateRoom(room.roomId, {
                id: room.roomId,
                number: String(room.roomNumber),
                building: room.roomBuilding || "",
      ...
      showToast(
        fail === 0
          ? `บันทึก ${ok} ห้อง เรียบร้อย`
          : `สำเร็จ ${ok} / ล้มเหลว ${fail} ห้อง`,
  ...
  const elecConstValue =
    utilityConsts.find(
      (c) =>
        c.subject?.toLowerCase().includes("electricity") ||
        c.subject?.includes("ไฟ"),
    )?.cost || 0;
  const waterConstValue =
    utilityConsts.find(
      (c) =>
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto justify-end">
            {priorityMeter && (
              <button
                ...
                className="flex-1 sm:flex-none flex items-center justify-center gap-1.5 px-3 sm:px-4 py-2.5 rounded-xl bg-indigo-50 text-indigo-600 border border-indigo-100 font-bold text-sm hover:bg-indigo-100 transition-all shrink-0"
              >
                {isTogglingPriority ? (
                  <Loader2 size={15} className="animate-spin" />
                ) : (
            ...
            <button
              onClick={() => setShowConstPanel(true)}
              className="flex-1 sm:flex-none flex items-center justify-center gap-1.5 px-3 sm:px-4 py-2.5 rounded-xl bg-gray-100 text-gray-600 border border-gray-200 font-bold text-sm hover:bg-gray-200 transition-all shrink-0"
            >
              <Settings size={15} />
              <span>อัตราคงที่</span>
            </button>
          </div>
        </div>
      ...
      >
        {selectedCount === 0 ? (
          <p className="text-center text-sm font-bold text-gray-400">
            เลือกห้องที่ต้องการตั้งค่าก่อน
          </p>
        ) : (
          <div className="max-w-2xl mx-auto flex items-center justify-between gap-3">
            <div>
              <p className="font-black text-[#f3a638] text-sm">
                เลือกแล้ว {selectedCount} ห้อง
              </p>
