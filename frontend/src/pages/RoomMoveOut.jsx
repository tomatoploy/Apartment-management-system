...
// ✅ ฟังก์ชันช่วยแปล Label เพื่อให้ชัวร์ว่าเป็นภาษาไทยเสมอ
const translateLabel = (label) => {
  if (!label) return "";
  let newLabel = label;
  // แปลงจาก CONSTANT_LABELS
  Object.keys(CONSTANT_LABELS).forEach((key) => {
    if (newLabel.includes(key)) {
      newLabel = newLabel.replace(key, CONSTANT_LABELS[key]);
    }
  });
  return newLabel;
};
...
const StepNotice = ({ mode, currentStep }) => {
  const notices = {
    1:
      mode === "absconded"
        ? "บิลค่าเช่าค้างชำระ (จะถูกบันทึกเป็นหนี้สูญ)"
        : "กรุณาเคลียร์บิลค่าเช่าค้างชำระ ด้วยการชำระเงินหรือหักจากเงินประกัน",
    2: "กรุณาระบุรายการทรัพย์สินที่เสียหาย ยอดจะถูกคำนวณเข้ารายการค่าเสียหาย",
  };
  ...
  return (
    ...
    >
      <div className="bg-white rounded-[32px] w-full max-w-lg shadow-2xl flex flex-col max-h-[90vh]">
        <div
          className={`flex items-center justify-between px-7 py-5 rounded-t-[32px] ${mode === "absconded" ? "bg-red-500" : "bg-[#f3a638]"}`}
        >
        ...
        <div className="overflow-y-auto flex-1 px-7 py-5 space-y-5">
          <div>
            <p className="text-sm font-black text-gray-500 mb-3 uppercase tracking-wider">
              รายการที่ต้องชำระ
            </p>
            <div className="space-y-2">
              {finalItems.map((item) => (
                <div
                  key={item.id}
                  className="flex justify-between items-center bg-gray-50/80 rounded-2xl px-5 py-3 border border-gray-100"
                >
          ...
          {mode !== "absconded" && deposits.length > 0 && (
            <div className="bg-blue-50/50 rounded-2xl px-5 py-4 border border-blue-100">
              <p className="text-[11px] font-black text-blue-500 mb-2 uppercase tracking-wider">
                เงินประกัน
              </p>
              {deposits.map((d) => (
                <div
          ...
          <div
            className={`rounded-2xl px-5 py-4 ${isRefund ? "bg-green-50 border border-green-200" : "bg-red-50 border border-red-200"}`}
          >
            <div
              className={`flex justify-between font-black text-lg ${isRefund ? "text-green-600" : "text-red-600"}`}
            >
              <span>
                {mode === "absconded"
                  ? "รวมยอดหนี้สูญ"
                  : isRefund
                    ? "ยอดสุทธิที่ต้องคืนผู้เช่า"
                    : "ยอดสุทธิที่ผู้เช่าต้องจ่ายเพิ่ม"}
              </span>
          ...
          {mode !== "absconded" && (
            <div className="bg-gray-100 p-6 rounded-[28px] border-2 border-dashed border-gray-200">
              <label className="text-sm font-black text-gray-600 flex items-center gap-2 mb-3 px-1">
                <Banknote size={18} className="text-orange-500" /> ยอดเงินที่
                {isRefund ? "จ่ายคืน" : "รับชำระ"}จริง
              </label>
              <div className="relative">
                <input
                  ...
                  placeholder="0.00"
                />
                <span className="absolute right-6 top-1/2 -translate-y-1/2 font-black text-gray-400">
                  ฿
                </span>
              </div>
              <p className="text-[10px] text-gray-400 font-bold mt-3 text-center uppercase tracking-wide">
                {isRefund
                  ? "* ยอดคืนเงินจะถูกบันทึกเป็นค่าติดลบในระบบบัญชี"
                  : "* ตรวจสอบยอดเงินให้ถูกต้องก่อนกดยืนยัน"}
              </p>
            </div>
          )}
          ...
          <div className="flex items-start gap-2 bg-yellow-50 border border-yellow-200 rounded-xl px-4 py-3">
            ...
            <p className="text-[11px] font-bold text-yellow-700 leading-relaxed">
              เมื่อกดยืนยัน ระบบจะปิดสัญญาและเปลี่ยนสถานะห้องเป็นว่างทันที
        ...
        <div className="flex gap-3 px-7 py-6 border-t border-gray-100">
          <button
            onClick={onClose}
            disabled={isConfirming}
            className="flex-1 py-3.5 rounded-2xl border-2 border-gray-200 font-bold text-gray-600 hover:bg-gray-50 disabled:opacity-50 transition-all"
          >
            ยกเลิก
          </button>
          <button
            onClick={() => onConfirm(finalItems, actualPaid, isRefund)}
            disabled={isConfirming}
          ...
          >
            {isConfirming ? (
              ...
              <>
                <CheckCircle2 size={18} /> ยืนยันย้ายออก
              </>
            )}
          </button>
        </div>
...
const CheckoutManager = () => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const activeContract = allContracts.find((c) => {
        ...
        return (
          Number(c.roomId || c.RoomId) ===
            Number(targetRoom.roomId || targetRoom.id) &&
          (cStatus === "active" ||
            cStatus === "reserved" ||
            cStatus === "expired")
        );
      ...
      setPropertyConstants(
        constants.filter((c) => c.category?.toLowerCase() === "property"),
      );

      const allPayments = extractArray(
        await paymentService.getPaymentsByContract(
          activeContract.id || activeContract.Id,
        ),
      );
      ...
      setUnpaidBills(
        unpaid.map((p) => {
          ...
          const getDetail = (noteStr, prefix) => {
            let d =
              noteStr?.match(new RegExp(`${prefix}:[^|]*`))?.[0]?.trim() || "";
            return d
          ...
          return {
            ...
            details: [
              ...(p.roomRate
                ? [{ label: "ค่าเช่าห้อง", amount: Number(p.roomRate) }]
                : []),
              ...(p.electricalCost
                ? [
                    {
                      label: elecDetail ? `ค่าไฟฟ้า ${elecDetail}` : "ค่าไฟฟ้า",
                      amount: Number(p.electricalCost),
                    },
                  ]
                : []),
              ...(p.waterCost
                ? [
                    {
                      label: waterDetail
                        ? `ค่าน้ำประปา ${waterDetail}`
                        : "ค่าน้ำประปา",
                      amount: Number(p.waterCost),
                    },
                  ]
                : []),
              ...(p.internetCost
                ? [
                    {
                      label: translateLabel("ค่าอินเทอร์เน็ต"),
                      amount: Number(p.internetCost),
                    },
                  ]
                : []),
              ...(p.laundryCost
                ? [
                    {
                      label: translateLabel("ค่าซักรีด"),
                      amount: Number(p.laundryCost),
                    },
                  ]
                : []),
              ...(p.additionalCost
                ? [
                    {
                      label:
                        translateLabel(p.additionalDetail) || "รายการเพิ่มเติม",
                      amount: Number(p.additionalCost),
                    },
                  ]
                : []),
            ],
              ...
              ? [
                  {
                    id: "cm-rent",
                    label: "ค่าเช่าห้อง",
                    amount: Number(curBill.roomRate),
                  },
                ]
              : []),
            ...(curBill.electricalCost
              ? [
                  {
                    id: "cm-elec",
                    label: elecDetail ? `ค่าไฟฟ้า ${elecDetail}` : "ค่าไฟฟ้า",
                    amount: Number(curBill.electricalCost),
                  },
                ]
              : []),
            ...(curBill.waterCost
              ? [
                  {
                    ...
                    label: waterDetail
                      ? `ค่าน้ำประปา ${waterDetail}`
                      : "ค่าน้ำประปา",
                    amount: Number(curBill.waterCost),
                  },
                ]
              : []),
            ...(curBill.internetCost
              ? [
                  {
                    id: "cm-inet",
                    label: translateLabel("ค่าอินเทอร์เน็ต"),
                    amount: Number(curBill.internetCost),
                  },
                ]
              : []),
            ...(curBill.laundryCost
              ? [
                  {
                    id: "cm-laundry",
                    label: translateLabel("ค่าซักรีด"),
                    amount: Number(curBill.laundryCost),
                  },
                ]
              : []),
            ...(curBill.additionalCost
              ? [
                  {
                    id: "cm-add",
                    label:
                      translateLabel(curBill.additionalDetail) ||
                      "รายการเพิ่มเติม",
                    amount: Number(curBill.additionalCost),
                  },
                ]
              : []),
          ],
  ...
  const getContractDisplayStatus = useMemo(() => {
    ...
    if (contract.endDate) {
      ...
      if (end < now || contract.status === "Expired")
        return "หมดสัญญาแล้ว (Expired)";
    }
    ...
    setAssets((prev) => {
      ...
      return [
        ...prev,
        {
          id: Date.now(),
          constantId: prop.id,
          // ✅ แปลงด้วย translateLabel และ || fallback เผื่อไว้
          label: `ค่าเสียหาย: ${CONSTANT_LABELS[prop.subject] || translateLabel(prop.subject)}`,
  ...
  const handleConfirmCheckout = async (
    ...
    try {
      ...
      const now = new Date();
      const calcResult = await paymentService
        .generatePayment(contractId, now.getFullYear(), now.getMonth() + 1)
        .catch(() => ({}));
      ...
      const cleanContract = {
        ...
        InitialElectricUnit:
          contract.initialElectricUnit || contract.InitialElectricUnit || 0,
        InitialWaterUnit:
          contract.initialWaterUnit || contract.InitialWaterUnit || 0,
      ...
      const cleanRoom = {
        id: Number(roomId),
        number: String(
          roomInfo.roomNumber || roomInfo.Number || roomInfo.number,
        ),
        building: String(
          roomInfo.roomBuilding || roomInfo.Building || roomInfo.building || "",
        ),
        floor: String(
          roomInfo.roomFloor || roomInfo.Floor || roomInfo.floor || "1",
        ),
        status: "available",
      ...
      const checkoutNote =
        mode === "absconded" ? "* (ผู้เช่าหนี)" : "* (ย้ายออก)";
      ...
      for (const bill of unpaidBills) {
        ...
        await paymentService.updatePayment(bill.paymentId, {
          note: updatedNote,
        });
      ...
      // ── บันทึกบิลเดือนปัจจุบัน ──────────────────────────────────────────
      {
        ...
        for (const item of finalConfirmedItems) {
          ...
          // ✅ เช็คจากคำที่แปลมาแล้วด้วย
          if (t === "rent" || lbl.includes("ค่าเช่าห้อง")) {
            roomRate += amt;
          } else if (t === "electric" || lbl.includes("ค่าไฟ")) {
            electricalCost += amt;
          } else if (t === "water" || lbl.includes("ค่าน้ำ")) {
            waterCost += amt;
          } else if (lbl.includes("อินเทอร์เน็ต") || lbl.includes("Internet")) {
            internetCost += amt;
          } else if (lbl.includes("ซักรีด") || lbl.includes("Laundry")) {
            laundryCost += amt;
          } else if (
            t === "asset" ||
            t === "damage" ||
            lbl.includes("ทรัพย์สิน") ||
            lbl.includes("เสียหาย") ||
            lbl.includes("เฟอร์นิเจอร์") ||
            lbl.includes("Furniture")
          ) {
            ...
            additionalCost += amt;
            if (item.label) additionalDetails.push(item.label);
          }
        }

        const totalFurnitureCost =
          propertyDamageTotal > 0
            ? propertyDamageTotal
        ...
        if (currentMonthBill) {
          const putPayload = {
            note: currentMonthBill.note
              ? `${currentMonthBill.note} ${billNote}`
              : billNote,
          };
      ...
      alert(
        mode === "absconded"
          ...
          : "ยืนยันการย้ายออกและบันทึกการเงินเรียบร้อย",
      );
      navigate("/rooms");
    } catch (err) {
  ...
  const totalExpenses = editedBillItems.reduce(
      ...
      <div className="max-w-full mx-auto px-4">
          ...
          <div className="flex gap-4">
            ...
            <div>
              ...
              <div className="text-xs md:text-sm text-gray-500 mt-1 font-bold">
                <p>
                  วันสิ้นสุดสัญญา :{" "}
                  {contract?.endDate ? toThaiDate(contract.endDate) : "-"}
                </p>
              </div>
            </div>
          </div>
          <span
            className={`px-4 py-1.5 text-xs font-black rounded-xl ${getContractDisplayStatus.includes("Expired") ? "bg-red-100 text-red-600" : "bg-green-100 text-green-600"}`}
          >
        ...
        <div className="md:border-[1.5px] md:border-blue-300 md:rounded-[40px] md:p-8 relative h-auto w-full max-w-5xl mx-auto">
          ...
          {currentStep === 1 && (
            <div className="space-y-4">
              {unpaidBills.length === 0 ? (
                <EmptyState message="ไม่มีบิลค้างชำระ" />
              ) : (
                  ...
                  >
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2 md:gap-6">
                        ...
                        <ChevronDown
                          className={`text-gray-400 transition-transform ${openBills[billGroup.period] ? "rotate-180" : ""}`}
                        />
                      </div>
                    </div>
                  ...
                  return (
                    ...
                    >
                      <div className="flex justify-between items-start w-full mb-3">
                        <div
                          className={`p-2 rounded-xl ${isSelected ? "bg-orange-200 text-orange-700" : "bg-gray-100 text-gray-400"}`}
                        >
                          <Box size={20} />
                        </div>
                        {isSelected && (
                      ...
                      <p
                        className={`font-black text-sm mb-1 ${isSelected ? "text-orange-900" : "text-gray-700"}`}
                      >
                        {translateLabel(prop.subject)}{" "}
                      </p>
                      <p
                        className={`text-xs font-bold ${isSelected ? "text-orange-600" : "text-gray-400"}`}
                      >
          ...
          {currentStep === 3 && (
            <div className="mt-4 space-y-4">
              ...
              <div className="flex flex-col items-center gap-4 mt-2">
                {!isEditStep3 ? (
                  <WhiteButton
                    label="แก้ไขรายการใบเสร็จ"
                    icon={Edit3}
                    onClick={() => setIsEditStep3(true)}
                    className="px-10"
                  />
                ) : (
                  <button
                    onClick={() => setIsEditStep3(false)}
                    className="px-8 py-2.5 rounded-2xl border-2 border-gray-200 text-gray-500 font-bold hover:bg-gray-50 transition-all"
                  >
                    ยกเลิกการแก้ไข
                  </button>
                )}
          ...
          {currentStep === 4 && (
            <div className=" mt-4">
              ...
              {/* <div className="flex justify-center mt-2 mb-4">
                {!isEditStep4 ? (
                  <WhiteButton
                    label="แก้ไขรายการเงินประกัน"
                    icon={Edit3}
                    onClick={() => setIsEditStep4(true)}
                    className="px-10"
                  />
                ) : (
                  <button
                    onClick={() => setIsEditStep4(false)}
                    className="px-8 py-2.5 rounded-2xl border-2 border-gray-200 text-gray-500 font-bold hover:bg-gray-50 transition-all"
                  >
                    ยกเลิกการแก้ไข
                  </button>
              ...
              <div className="p-2 bg-gray-50 rounded-2xl border border-gray-100 text-center">
                <p className="text-xs font-bold text-blue-400">
                  * ยอดเงินประกันจะถูกนำไปหักลบกับยอดหนี้ในขั้นตอนสุดท้าย
                </p>
              </div>
            </div>
          )}
          ...
          {currentStep === 5 && (
            <div className="space-y-6">
              <div
                className={`md:rounded-2xl md:overflow-hidden md:border md:shadow-sm ${mode === "absconded" ? "md:border-gray-200" : "md:border-gray-200"}`}
              >
                <div
                ...
                <div className="md:p-6 space-y-5 bg-gray-50/50">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="bg-white md:rounded-[24px] md:border md:border-gray-200 md:p-5 shadow-sm">
                      <h3 className="text-base font-black text-blue-600 mb-3 flex items-center gap-2">
                      ...
                      <div className="flex justify-between font-black text-blue-600 text-lg pt-1 border-t border-gray-300 mt-0.5">
                        <span>รวมเงินประกัน</span>
                        <span>{totalDeposit.toLocaleString()} ฿</span>
                      </div>
                    </div>
                    <div className="bg-white md:rounded-[24px] md:border md:border-gray-200 md:p-5 shadow-sm">
                      <h3 className="text-base font-black text-red-500 mb-3 flex items-center gap-2">
                      ...
                      <div className="flex justify-between font-black text-red-500 text-lg pt-1 border-t border-gray-300 mt-0.5 ">
                  ...
                  {/* {mode !== "absconded" && (
                    <div
                      className={`p-6 md:p-8 text-center md:rounded-[24px] md:border border-t-4 ${isRefund ? "bg-green-50 border-green-200" : "bg-red-50  border-red-200"}`}
                    >
                      ...
                      >
                        {Math.abs(netAmount).toLocaleString()} บาท
                      </p>
                    </div>
                  )} */}
                </div>
