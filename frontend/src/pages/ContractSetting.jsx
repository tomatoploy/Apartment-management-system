...
const initialContractTemplates = [
  {
    id: 1,
    name: "สัญญาเช่ามาตรฐาน (รายปี)",
    content: `
      <h1 style="text-align: center;">สัญญาเช่าห้องพัก</h1>
      <p>ทำขึ้น ณ {{building_name}} วันที่ {{contract_date}}</p>
      <p>ข้าพเจ้า <strong>{{landlord_name}}</strong> (ผู้ให้เช่า) ตกลงให้เช่าห้องพักเลขที่ <strong>{{room_number}}</strong> แก่ <strong>{{tenant_name}}</strong> (ผู้เช่า)</p>
      <p>โดยมีรายละเอียดค่าเช่าเดือนละ <strong>{{monthly_rent}}</strong> บาท ({{monthly_rent_text}})</p>
      <p><strong>เงินประกัน:</strong> {{deposit}} บาท</p>
      <p><strong>ข้อตกลงเพิ่มเติม:</strong> ห้ามสูบบุหรี่ในห้องพักและห้ามเลี้ยงสัตว์</p>
    `,
    is_active: true,
  },
  {
    id: 2,
    name: "สัญญาเช่าระยะสั้น (6 เดือน)",
    content: `<h1>สัญญาเช่าระยะสั้น</h1><p>สัญญาสำหรับห้อง {{room_number}} เริ่มวันที่ {{start_date}}...</p>`,
    is_active: false,
  },
];
...
const ContractTemplate = () => {
  ...
  const availableVariables = [
    ...
    { key: "{{monthly_rent}}", desc: "ค่าเช่ารายเดือน", example: "5,000" },
    { key: "{{deposit}}", desc: "เงินประกัน", example: "10,000" },
    {
      key: "{{start_date}}",
    ...
    },
    {
      key: "{{building_name}}",
      desc: "ชื่ออาคาร",
      example: mockSettings.building_name,
    },
    {
      key: "{{landlord_name}}",
      desc: "ชื่อผู้ให้เช่า",
      example: mockSettings.landlord_name,
    },
  ];
        ...
        <div className="lg:col-span-4 space-y-4">
          ...
          {templates.map((t) => (
            ...
            >
              ...
              <span
                className={`text-[10px] uppercase tracking-wider font-bold px-2 py-1 rounded-full mt-2 inline-block ${t.is_active ? "bg-green-100 text-green-600" : "bg-gray-200 text-gray-500"}`}
              >
        ...
        <div className="lg:col-span-8 bg-white rounded-3xl shadow-sm border border-gray-200 p-6 ">
          ...
          <div className="mt-10 p-4 bg-amber-50 rounded-2xl border border-amber-100 flex gap-3">
            <p className="text-sm text-amber-900">
              เมื่อสร้างสัญญาจากหน้าห้อง ระบบจะแทนที่ตัวแปรที่เป็น
