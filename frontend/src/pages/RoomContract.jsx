...
import { Calendar, CreditCard, Edit3, Save, Zap, Droplets, XCircle, FileText, FileSignature, Download, History, CheckCircle2, Clock, Printer, Eye, X, Loader2, Scissors, Undo, Redo } from "lucide-react";
...
const EditorToolbar = memo(({ currentSize, onSizeChange, onInsertPageBreak }) => {
  ...
  return (
    <div className={`fixed bottom-8 left-1/2 -translate-x-1/2 z-[300] flex items-center gap-2.5 px-5 py-3 rounded-2xl shadow-xl font-bold text-sm whitespace-nowrap transition-all animate-slide-up ${type === "success" ? "bg-gray-900 text-white" : "bg-red-500 text-white"}`}>
      {type === "success"
        ...
        : <span size={16} className="shrink-0">⚠</span>
      } {message}
    </div>
  );
});
...
const RoomContract = () => {
  ...
  const fetchContract = async () => {
    ...
    try {
      ...
      if (activeContract) {
        ...
        if (cElec) { setElecMode("custom"); setElecRate(cElec); } 
        else { setElecMode("constant"); setElecRate(""); }

        if (cWater) { setWaterMode("custom"); setWaterRate(cWater); } 
        else { setWaterMode("constant"); setWaterRate(""); }
  ...
  useEffect(() => { fetchContract(); }, [roomNumber]);
  ...
  useEffect(() => {
    ...
    html = html.replace(/\{วันที่ทำสัญญาแบบเต็ม\}/g,
      insertBlackText(`${today.getDate()} ${getThaiMonthName(today)} ${today.getFullYear() + 543}`));
    html = html.replace(/\{วันทำสัญญา\}/g, insertBlackText(today.getDate()));
    html = html.replace(/\{เดือนทำสัญญา\}/g, insertBlackText(getThaiMonthName(today)));
    html = html.replace(/\{ปีทำสัญญา\}/g, insertBlackText(today.getFullYear() + 543));
    ...
    const tName = tenantData
      ...
      : "...........................";
    const tAddress = tenantData?.address || tenantData?.Address || "...........................";
    const tPhone = tenantData?.phone || tenantData?.Phone || "...........................";
    const tNin = tenantData?.nin || tenantData?.Nin || "...........................";
    ...
    html = html.replace(/\{หมายเลขชั้นของห้องพัก\}/g, insertBlackText(fullRoom?.roomFloor || "......"));

    const sDate = formData.startDate ? toThaiDate(formData.startDate) : "...........................";
    const eDate = formData.endDate ? toThaiDate(formData.endDate) : "...........................";
    ...
    html = html.replace(/\{เลขมิเตอร์ไฟเข้าพัก\}/g, insertBlackText(formData.initialElectricUnit || "0"));
    html = html.replace(/\{เลขมิเตอร์น้ำเข้าพัก\}/g, insertBlackText(formData.initialWaterUnit || "0"));
    
    html = html.replace(/<mark[^>]*>/gi, "");
    html = html.replace(/<\/mark>/gi, "");

    setPreviewHtml(html);
  }, [templateHtml, formData, apartmentData, tenantData, fullRoom, adminName, roomNumber]);
                  ...
                  <div className="p-6 md:p-8 flex flex-col md:grid md:grid-cols-2 gap-x-8 gap-y-6">
                    <div className="space-y-6 order-1">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <InputGroup label="วันเริ่มสัญญา" name="startDate" type="date" value={formData.startDate} onChange={handleChange} disabled={!isEditing} />
                    ...
                    <div className="space-y-6 order-2">
                      ...
                      <div className={`p-5 rounded-2xl border space-y-4 ${isRenewing ? "bg-green-50/50 border-green-100" : "bg-orange-50/50 border-orange-100"}`}>
                        ...
                        <InputGroup label="ไฟเริ่มต้น (หน่วย)" name="initialElectricUnit" type="number" value={formData.initialElectricUnit} onChange={handleChange} disabled={!isEditing} />
                        <InputGroup label="น้ำเริ่มต้น (หน่วย)" name="initialWaterUnit" type="number" value={formData.initialWaterUnit} onChange={handleChange} disabled={!isEditing} />
                      </div>
                    </div>
                      ...
                      <div className="flex flex-col gap-4">
                        <div className="p-4 md:p-5 bg-blue-50 border border-blue-100 rounded-2xl flex flex-col md:flex-row items-start md:items-center justify-between shadow-sm gap-4">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <p className="text-base font-bold text-gray-800">ตรวจสอบและพิมพ์เอกสาร</p>
                              <p className="text-[11px] md:text-xs font-bold text-gray-500 mt-0.5">กดที่ปุ่มเพื่อตรวจสอบแก้ไขข้อความ ก่อนสั่งเซฟเป็นไฟล์ PDF</p>
                            </div>
                          </div>
                          <div className="flex gap-2 shrink-0 w-full md:w-auto">
                            ...
                            >
                              <Eye size={18} strokeWidth={2.5} /> ดูตัวอย่างและพิมพ์
                            </button>
                          </div>
                        </div>

                        <div className="flex flex-col gap-1 mt-2">
                          <label className="text-[13px] font-bold text-gray-500 ml-1">หมายเหตุในระบบ (เฉพาะผู้ดูแลเห็น)</label>
                          <textarea
                            name="Note"
                            value={formData.Note}
                            onChange={handleChange}
            ...
            <div className="flex justify-between items-center px-6 py-4 bg-white border-b border-gray-200 shrink-0">
              <div>
                <h3 className="text-lg font-black text-gray-800 flex items-center gap-2">
                  <FileText size={20} className="text-blue-500"/> ตรวจสอบและแก้ไขสัญญาก่อนพิมพ์
                </h3>
                <p className="text-xs font-bold text-gray-500 mt-0.5">ห้อง {roomNumber}</p>
              </div>
              <div className="flex items-center gap-2">
                ...
                >
                  <X size={20} strokeWidth={3} className="text-gray-500 hover:text-red-500" />
                </button>
              </div>
            </div>
...
const InputGroup = ({ label, name, type, value, onChange, disabled }) => (
  <div className="flex flex-col gap-1">
    ...
    <input type={type} name={name} value={value} onChange={onChange} disabled={disabled}
      className={"w-full p-3 border rounded-xl font-medium focus:outline-none transition-all " + (disabled ? "bg-gray-50 border-gray-100 text-gray-500 cursor-not-allowed" : "bg-white border-gray-300 text-gray-800 focus:border-[#f3a638] focus:ring-1 focus:ring-[#f3a638]")}
    />
  </div>
);
