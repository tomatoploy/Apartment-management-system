...
const Rooms = () => {
  ...
  useEffect(() => {
    const loadAllData = async () => {
      try {
        ...
        const rawRooms = Array.isArray(roomData)
          ? roomData
          : (roomData?.$values ?? []);
        ...
        const normalized = rawRooms.map((room) => {
          ...
          if (room.ContractEndDate) {
            const diffDays =
              (new Date(room.ContractEndDate) - today) / (1000 * 60 * 60 * 24);
          ...
          const roomRequests = activeRequests.filter(
            (req) => req.roomNumber === room.roomNumber,
          );

          roomRequests.forEach((req) => {
            const subject = req.subject?.toLowerCase();
            if (subject === "fix" && !icons.includes("fix")) icons.push("fix");
            else if (subject === "clean" && !icons.includes("clean"))
              icons.push("clean");
            else if (subject === "leave" && !icons.includes("leave"))
              icons.push("leave");
            else if (subject === "other" && !icons.includes("other"))
              icons.push("other");
          });

          // --- Logic 3: Parcels ---
          const hasActiveParcel = activeParcels.some(
            (parcel) => parcel.roomNumber === room.roomNumber,
          );
          if (hasActiveParcel) {
            icons.push("package");
          }
  ...
  const floors = useMemo(
    () =>
      [...new Set(roomsData.map((r) => r.floor))].sort(
