...
import { 
  Search, Filter, HelpCircle, X, LogIn, LogOut, Wrench, Sparkles, 
  Package, Clock, FileText, RotateCw, Loader2, AlertTriangle, ChevronDown, ChevronUp,
} from "lucide-react";
    ...
    <div ref={ref} className="bg-gray-50 p-6 rounded-3xl border border-gray-100">
      ...
      {!isVisible ? (
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-6">
          {rooms.map((room) => (
            <div key={room.roomId || room.id} className="h-24 rounded-2xl bg-gray-200 animate-pulse" />
          ))}
        </div>
      ) : (
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-6">
            ...
            <div className="px-5 pb-4 flex flex-wrap gap-2">
              {expiredRooms.map((r) => (
                <div key={r.roomId}
                  ...
                  <span className="font-black text-red-600">{r.building}{r.roomNumber}</span>
                  {r.tenantFirstName && (
                    <span className="text-gray-500 font-medium">{r.tenantFirstName}</span>
                  )}
            ...
            <div className="px-5 pb-4 flex flex-wrap gap-2">
              {urgentRooms.map((r) => (
                <div key={r.roomId}
                  ...
                  <span className="font-black text-orange-600">{r.building}{r.roomNumber}</span>
                  {r.tenantFirstName && (
                    <span className="text-gray-500 font-medium">{r.tenantFirstName}</span>
                  )}
...
const Rooms = () => {
  ...
  useEffect(() => {
    const loadAllData = async () => {
      setIsLoading(true);
      try {
        const [roomData, requestData, parcelData, paymentData, contractData] = await Promise.all([
        ...
        const normalized = rawRooms.map((room) => {
          ...
          // ── icons ─────────────────────────────────────────────
          const roomStatus = (room.roomStatus || room.status || "").toLowerCase();
          ...
          roomRequests.forEach((req) => {
            ...
            if (["fix", "clean", "leave", "other"].includes(sub) && !icons.includes(sub))
              icons.push(sub);
          });
  ...
  const filteredRoomsByFloor = useMemo(() => {
    ...
    roomsData.forEach((room) => {
      const matchesIcon     = deferredIcon.length === 0 || deferredIcon.some((i) => room.icons.includes(i));
      const matchesStatus   = deferredStatus.length === 0 || deferredStatus.includes(room.status);
      const matchesBuilding = deferredBuilding === "ALL" || room.building === deferredBuilding;
    ...
    Object.values(result).forEach((arr) =>
      arr.sort((a, b) => a.roomNumber.localeCompare(b.roomNumber, "th", { numeric: true }))
    );
    return result;
  }, [roomsData, deferredIcon, deferredStatus, deferredBuilding, deferredSearch]);
  ...
  return (
    <>
      ...
      {/* ── ✅ Contract Warning Banner ── */}
      {!isLoading && (
        <ContractWarningBanner expiredRooms={expiredRooms} urgentRooms={urgentRooms} />
      )}
      ...
      {isLoading ? (
        <div className="py-32 flex flex-col items-center justify-center gap-4">
          ...
          <p className="text-gray-500 font-bold animate-pulse">กำลังโหลดผังห้อง...</p>
        </div>
      ) : (
        ...
        <div className="mb-2">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            {[
              ...
              <button key={item.id} onClick={() => toggleIconFilter(item.id)}
                className={`py-3 rounded-xl text-base font-bold transition-all border-2
...
const LegendModal = ({ onClose }) => (
  <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4"
    ...
    <div className="bg-white rounded-[40px] w-full max-w-3xl p-8 relative shadow-2xl">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-6 gap-x-12 px-2">
        ...
        <IconDetail icon={<LogIn className="text-green-600" />}    text="มีการจอง / วันย้ายเข้า" />
        <div className="flex items-start gap-3">
          <div className="p-1 bg-orange-100 rounded-md shrink-0"><Clock className="text-orange-500" size={26} /></div>
          <div>
            <p className="font-bold text-[18px] text-gray-700">ใกล้ครบสัญญา</p>
            <p className="text-[14px] text-gray-500">เหลือน้อยกว่า 30 วัน (badge ส้ม)</p>
          </div>
        </div>
        <div className="flex items-start gap-3">
          <div className="p-1 bg-red-100 rounded-md shrink-0"><Clock className="text-red-600" size={26} /></div>
          <div>
            <p className="font-bold text-[18px] text-gray-700">สัญญาหมดอายุแล้ว</p>
            <p className="text-[14px] text-gray-500">เลยวันสิ้นสุดสัญญา (badge แดง)</p>
          </div>
        </div>
      </div>
    </div>
