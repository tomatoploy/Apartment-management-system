...
//         // => optionsBuilder.UseMySql("server=100.71.138.33;port=3306;user=nongnut;password=Password123!;database=Dormitory", Microsoft.EntityFrameworkCore.ServerVersion.Parse("11.8.2-mariadb"));
    ...
    // {
    //     // เรียกใช้ลอจิกเปลี่ยนสถานะห้องก่อนเซฟ
    //     UpdateRoomStatusLogicSync();
    ...
    // {
    //     // เรียกใช้ลอจิกเปลี่ยนสถานะห้องก่อนเซฟ
    //     await UpdateRoomStatusLogicAsync(cancellationToken);
    //     return await base.SaveChangesAsync(cancellationToken);
    // }

    // private async Task UpdateRoomStatusLogicAsync(CancellationToken ct)
    // {
    //     // ดึงเฉพาะ Contract ที่ถูกเพิ่ม (Insert) หรือแก้ไข (Update)
    //     var entries = ChangeTracker.Entries<Contract>()
    ...
    //         // ดึงข้อมูลห้องที่ผูกกับสัญญานี้
    //         var room = await Room.FindAsync(new object[] { contract.RoomId }, ct);
    //         if (room == null) continue;

    //         // ---------------------------------------------------
    //         // เทียบเท่า TRIGGER: after_contract_insert
    //         // ---------------------------------------------------
    ...
    //         }
    //         // ---------------------------------------------------
    //         // เทียบเท่า TRIGGER: after_contract_update
    //         // ---------------------------------------------------
    ...
    //                 room.Status = "occupied";
    //             }
    //             else if ((newStatus == "Terminated" || newStatus == "cancle") && 
    //                     (oldStatus != "Terminated" && oldStatus != "cancle"))
    //             {
    //                 room.Status = "available";
    //             }
    //             else if (newStatus == "Reserved" && oldStatus != "Reserved")
    //             {
    //                 room.Status = "reserved";
    //             }
    //         }
    ...
    // private void UpdateRoomStatusLogicSync()
    // {
    //     // ทำงานเหมือนตัว Async ด้านบน แต่ใช้สำหรับ .SaveChanges() ปกติ
    //     var entries = ChangeTracker.Entries<Contract>()
    //         .Where(e => e.State == EntityState.Added || e.State == EntityState.Modified)
    //         .ToList();
    ...
    //         var room = Room.Find(contract.RoomId);
    //         if (room == null) continue;
